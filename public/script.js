// JavaScript controller for the UIUC Study Spots SPA
// ---------------------------------------------
// Updated to
// 1. Repair the day‑forecast graph rendering.
// 2. Support a Thanksgiving break week.
// 3. Adjust semester end to the Friday of week 16.
...
function getCurrentAcademicTime(now) {
    ...
    for (const sem of SEMESTERS) {
        ...
        const totalCal = sem.weeks + (sem.has_spring_break ? 1 : 0) + (sem.has_thanksgiving_break ? 1 : 0);
        end.setDate(end.getDate() + totalCal * 7);
        // shift end to Friday of week 16
    ...
    const maxCal = current.weeks + (current.has_spring_break ? 1 : 0) + (current.has_thanksgiving_break ? 1 : 0);
    ...
    if (current.has_thanksgiving_break) {
        const tbw = current.thanksgiving_break_calendar_week;
        if (calWeek >= tbw) dataWeek = Math.max(1, dataWeek - 1);
    }
...
async function dateToWeekInfo(dateStr) {
    ...
    for (const sem of SEMESTERS) {
        ...
        if (ts >= start.getTime() && ts < end.getTime()) {
            const daysSince = Math.floor((ts - start.getTime()) / 86400000);
            let calWeek = Math.max(1, Math.floor(daysSince / 7) + 1);
            const maxCal = sem.weeks + (sem.has_spring_break ? 1 : 0) + (sem.has_thanksgiving_break ? 1 : 0);
            calWeek = Math.min(calWeek, maxCal);
            ...
            if (sem.has_thanksgiving_break) {
                const tbw = sem.thanksgiving_break_calendar_week;
                if (calWeek >= tbw) dataWeek = Math.max(1, dataWeek - 1);
            }
...
function navigateTo(viewId) {
    ...
    if (viewId === 'view-calendar-in') {
        // Ensure a building is selected before rendering detail
        if (!selectedBuilding) {
            // Fallback to first built from API when available
            const fallback = allBuildings.length ? allBuildings[0] : null;
            if (fallback) selectedBuilding = fallback;
        }
        updateDayDetail();
    }
}
...
async function renderMonthlyCalendar() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        const info = await dateToWeekInfo(dateStr);
        if (info.inSemester) {
            semesterDays.push({ date: dateStr, week: info.week, day: info.day });
        }
    }

    // Build demand map per (week,day) combo
    const demandMap = {};
    if (semesterDays.length) {
        const uniqueCombos = [...new Set(semesterDays.map(d => `${d.week}-${d.day}`))];
        for (const combo of uniqueCombos) {
            const [weekStr, day] = combo.split('-');
            const week = parseInt(weekStr, 10);
            try {
                const res = await fetch(`/api/demand?week=${week}&day=${day}&hour=12`);
                const data = await res.json();
                if (data.predictions) {
                    const preds = data.predictions.map(p => p.Demand_Prediction).filter(v => v >= 0);
    ...
    for (let i = 0; i < startDayOfWeek; i++) {
        ...
        const dayNum = lastDayPrev - startDayOfWeek + 1 + i;
...
async function renderHourlyBars() {
    ...
    if (!selectedBuilding) {
        chart.innerHTML = '<p style="color:#c00;padding:20px;">Select a building first.</p>';
        return;
    }
    try {
        ...
        for (let h = 0; h < 24; h++) {
            let demand = -1;
            if (dayData && typeof dayData[h] === 'number') {
                demand = dayData[h];
            }
            const percent = demand < 0 ? 0 : Math.round(demand * 100);
            const height = demand < 0 ? 2 : Math.max(2, percent);
            ...
            container.onclick = () => {
                ...
                document.getElementById('hour-display-in').textContent = `${h}:00`;
                updateDayDetail();
            };
